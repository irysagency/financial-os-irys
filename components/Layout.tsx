...
const NavItem = React.memo(function NavItem({
  page, icon: Icon, label, activePage, onNavigate, onClose
}: NavItemProps) {
...
export const Layout: React.FC<LayoutProps> = ({ children, activePage, onNavigate }) => {
  ...
  const closeMenu = React.useCallback(() => setIsMobileMenuOpen(false), []);
    ...
    <div className="min-h-screen bg-main flex text-white font-sans selection:bg-[#FF4D00] selection:text-white">
      ...
      <aside className={`fixed md:sticky top-0 h-screen w-72 bg-card border-r border-[#2A2A2A] flex flex-col z-50 transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="flex-1 px-4 space-y-1 overflow-y-auto">
          ...
          <NavItem page="dashboard"    icon={LayoutDashboard} label="Dashboard"    activePage={activePage} onNavigate={onNavigate} onClose={closeMenu} />
          <NavItem page="transactions" icon={History}         label="Transactions" activePage={activePage} onNavigate={onNavigate} onClose={closeMenu} />
          <NavItem page="revenus"      icon={TrendingUp}      label="Revenus"      activePage={activePage} onNavigate={onNavigate} onClose={closeMenu} />
          <NavItem page="pnl"          icon={BarChart2}       label="P&L"          activePage={activePage} onNavigate={onNavigate} onClose={closeMenu} />
          <NavItem page="abonnements"  icon={Repeat}          label="Abonnements"  activePage={activePage} onNavigate={onNavigate} onClose={closeMenu} />

          <div className="text-xs font-semibold text-muted px-4 mt-8 mb-2 uppercase tracking-wider">Preferences</div>
          <NavItem page="settings" icon={Settings} label="Settings" activePage={activePage} onNavigate={onNavigate} onClose={closeMenu} />
        </div>
      ...
      {/* MOBILE OVERLAY */}
      {isMobileMenuOpen && (
        <div onClick={closeMenu} className="fixed inset-0 bg-black/50 z-40 md:hidden backdrop-blur-sm" />
      )}
